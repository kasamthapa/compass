...
export function EveningReviewDialog({ isOpen, onClose, today, tomorrow }: EveningReviewDialogProps) {
  ...
  // clobbered by the (stale) saved value.
  useEffect(() => {
    if (!isOpen) return
    if (review) {
  ...
  const steps: ReviewStep[] = [
    {
      ...
      render: () => <ScoreStep prompt="How was today?" score={score} onSelect={(value) => void selectScore(value)} />,
    },
    {
      render: () => (
        <div>
      ...
      render: () => (
        <div>
          ...
          {tomorrowMits.length < 3 ? (
            ...
            <p className="mt-2 min-h-11 py-2.5 text-subhead text-text-muted">Three is enough for tomorrow.</p>
          )}
        </div>
      ),
    },
  ...
  return (
    <ReviewDialog
      ...
      title="Close the day"
      steps={steps}
      resumeStep={resumeStepFor(review)}
      completionMessage="Day closed."
      onFinish={async () => {
